...
按原文顺序生成：一句大白话定义、短条目要点、3–5 个真实场景例子、易错易混、上场前检查清单、3–7 道快问快答（每题必须附参考答案）。短条目服务于使用前 5 分钟回看，不把长篇简报复制过来。答案写进 `10-cheatsheet.md` 事实源，HTML 只渲染、不得在视图层补写答案。
...
`--bootstrap` 和回灌都不能覆盖已有历史行。每轮使用 `<HHMMSS>-<topic-slug>` 作为 slug，并以 `<YYYY-MM-DD>-<slug>` 建立运行目录；同主题的新轮次保留新的时间化 slug。若目标目录已存在，在 slug 后追加两位序号，便于追溯且绝不覆盖历史产物。
