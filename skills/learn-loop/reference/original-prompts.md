...
执行时只替换 `{{主题}}`、`{{角色}}`、`{{水平}}`、`{{目标}}`；十段正文实际使用 `{{主题}}` 与 `{{角色}}`，`{{水平}}`/`{{目标}}` 由纪律层（`reference/curriculum.md` 第 6/7 步）消费。下面的十段保留原文指令；纪律层可以追加约束和产出格式，但不得覆盖这些要求。第 8、9 步在模式 A 中只做材料化，真实对话留给模式 B/C。
