...
- 画像确认问答是否为用户原话、`用户开场已提供`与诊断探针记录是否与字段标注自洽；
- 分歧、共识和盲区的语义判断是否合理；
- 评分和教学内容是否有质量；
