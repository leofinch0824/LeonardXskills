#!/usr/bin/env python3
"""Resolve a learn-loop workspace, bootstrap missing runtime files, and report capability fallbacks."""
...
def inspect_capabilities(warnings):
    retrieval_explicit = os.environ.get("LEARN_LOOP_RETRIEVAL_AVAILABLE")
    retrieval_declared = env_flag("LEARN_LOOP_RETRIEVAL_AVAILABLE")
    retrieval_verified = retrieval_declared and env_flag("LEARN_LOOP_RETRIEVAL_VERIFIED")
    retrieval_source = "LEARN_LOOP_RETRIEVAL_AVAILABLE" if retrieval_explicit else "宿主未声明"
    if retrieval_declared and not retrieval_verified:
        warnings.append("检索工具只有环境声明，未验证；第 1–5 步仍进入未锚定模式并将来源标 C")
    elif not retrieval_declared:
        warnings.append("未检测到检索工具声明；第 1–5 步进入未锚定模式并将来源标 C")

    declared_mode = os.environ.get("LEARN_LOOP_SUBAGENT_MODE", "").strip()
    if declared_mode not in {"parallel", "serial-isolated", "orchestrated"}:
        declared_mode = "orchestrated"
    subagents_declared = "LEARN_LOOP_SUBAGENT_MODE" in os.environ
    subagents_verified = subagents_declared and env_flag("LEARN_LOOP_SUBAGENT_VERIFIED")
    effective_mode = declared_mode if subagents_verified else "orchestrated"
    if subagents_declared and not subagents_verified:
        warnings.append("subagent 只有环境声明，未验证；建议记录 orchestrated 降级档位")
    elif not subagents_declared:
        warnings.append("未检测到独立 subagent 声明；建议记录 orchestrated 降级档位")

    return {
        "retrieval": {
            "declared": retrieval_declared,
            "verified": retrieval_verified,
            "available": retrieval_verified,
            "source": retrieval_source,
        },
        "subagents": {
            "declared": subagents_declared,
...
def build_report(args):
    ...
    capabilities = inspect_capabilities(warnings)
    if errors:
        status = "blocked"
    elif capabilities["retrieval"]["available"] and capabilities["subagents"]["available"]:
        status = "ready"
    else:
        status = "degraded"
    return {
