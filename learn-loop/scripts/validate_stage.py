...
def is_sentinel(value):
    ...
    return normalized in {"", "—", "-", "TBD"} or normalized.startswith(
        ("待填写", "待检索", "待真实", "待评分")
    )
...
def validate_stage_10(text, run_dir):
    ...
    qa_count = len(re.findall(r"(?m)^\s*\d+[.)、]\s+\S", section_content(text, "快问快答")))
    if not 3 <= qa_count <= 7:
...
def validate_exam_record(text):
    ...
    cursor_match = re.search(r"(?:当前游标|已完成题数)\s*[:：]\s*(\d+)", text)
...
def validate_html(run_dir):
    ...
    for path in html_files:
        ...
        if "交互施考未进行" not in text and "交互施考" not in text:
            violations.append(f"HTML 缺少施考状态行：{path.name}")
    return violations
