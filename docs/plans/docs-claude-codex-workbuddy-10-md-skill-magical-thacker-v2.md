...
- **模式 A · 生成流水线**（默认）：开场两问 → 自动跑完十步（第 8/9 步产出题库与费曼讲解材料，即"备课"）→ 交付 markdown 产物集 + 教科书式单文件 HTML。
...
| 交付物 | markdown 产物集（事实源）+ 单文件 HTML（教科书式视图，v1 交付） |
...
├── assets/
│   └── template.html             # 教科书式单文件模板：侧边栏(总览+十步+收尾)、三栏语义、翻卡题库、{{...}}占位
├── reference/
...
│   ├── 10-cheatsheet.md
│   ├── run-state.md              # 学习画像(两问答案) + 十步进度 + 独立性档位 + 锚定模式 + 练习状态 + 考试游标
...
| stage | 硬约束 |
|---|---|
| 0 | run-state 含学习画像、独立性档位、锚定模式三字段非空 |
...
python3 learn-loop/scripts/validate_stage.py --run-dir <run-dir> --all --json
```
...
1. `tests/test_learn_loop.py`：先写包契约、法条保真、markdown 契约测试（失败态），锁住结构
2. `reference/original-prompts.md`：从原文逐字抽取十段提示词、挖占位符（法条层，先行冻结）
3. `SKILL.md`（不变量 + 路径契约 + 模式 A 十步闸门 + 模式 B/C/复习协议入口）+ `agents/openai.yaml`
4. 5 份纪律层 reference（`perspectives.md`、`conflict.md`、`examination.md` 是重心）
5. 15 份 templates（十步产物 + exam-record/feynman-record + run-state/learner-profile/review-queue）
6. `scripts/preflight.py`（复用 paper-trail 路径解析与 bootstrap 结构，增加能力探测与建议档位）
7. `scripts/validate_stage.py` + `scripts/review_queue.py`，补齐单测与 fixtures
8. `assets/template.html` + `reference/html-guide.md`（可参考 ten-step-learning 的侧边栏/三栏/翻卡结构自研实现，不复用其 DESIGN 色板，用中性样式；单文件、无外部依赖）
9. 全量测试 + CLI smoke test
10. 更新 `README.md`：内容一览加一行、skills badge 1→2、仓库结构补 `learn-loop/`
