...
Intake 是用户确认的范围合同，也是唯一的计划内人工交互点：

- 用户确认后，将状态置为`已确认并冻结`；原始痛点和原子问题不得被后续阶段静默修改。
- Landscape 发现同义词或 taxonomy 节点时，将其写入 `01-landscape.md` 的“Intake 词表校准”，不改写 Intake。
- Landscape 发现原子问题的含义、边界或拆分方式错误时，将 Intake 状态置为`重新打开`，写明原因，并再次请求用户确认。
- 重新确认后恢复`已确认并冻结`，保留用户修改记录。
...
- 用户确认后状态为`已确认并冻结`；若为`重新打开`则不得进入 Discovery
